...
public class VideoEncoder implements Runnable, IVideoEncoder {
    ...
    private SurfaceTextureManager mTextureManager;
    ...
    private long mMaxFileSizeInBytes;
    ...
    public void prepare(AVMediaMuxer muxer) throws IOException, IllegalStateException {
        ...
        mVideoThread = new Thread(this);
        mVideoThread.setPriority(Thread.MAX_PRIORITY);
        mVideoThread.setName(TAG);
        mVideoThread.start();
    ...
    public void reset() {
        ...
        if (mInputSurface != null) throw new IllegalStateException();
        if (mMuxer != null) throw new IllegalStateException();
        if (mTextureManager != null) throw new IllegalStateException();
        if (mVideoThread != null) throw new IllegalStateException();
        if (mSurface != null) throw new IllegalStateException();
    ...
    private void prepareEncoder() throws IOException {
        ...
        int degrees = mMuxer.getOrientationHint();
        int width = mWidth;
        int height = mHeight;
...
//        }
    ...
    private void prepareSurfaceTexture() {
        mTextureManager = new SurfaceTextureManager();
        SurfaceTexture surfaceTexture = mTextureManager.getSurfaceTexture();
        ...
        try {
            ...
            while (true) {
                ...
                switch (mState) {
                    case Record:
                        drainEncoder(false);
                        if (mTextureManager.checkFrameAvailable()) {
                            mTextureManager.drawImage();
                            long presentTimeNs = System.nanoTime();
                            if (startTimestampNs == -1) {
                                startTimestampNs = presentTimeNs - lastTimestampNs;
                            }
                            lastTimestampNs = presentTimeNs - startTimestampNs;
                            mInputSurface.setPresentationTime(lastTimestampNs);
                            if (VERBOSE) Log.d(TAG, "Sending frame to encoder with pts (" + lastTimestampNs + ")");
    ...
    private void drainEncoder(boolean endOfStream) {
        ...
        while (true) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                mMuxer.setVideoMediaFormat(newFormat);
                mMuxer.start();
                ...
                if (mBufferInfo.size != 0) {
                    ...
                    mMuxer.writeVideoSampleData(encodedData, mBufferInfo);
    ...
    private static class SurfaceTextureManager
         ...
         */
        public boolean checkFrameAvailable() {
